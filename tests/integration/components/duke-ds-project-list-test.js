...
module('Integration | Component | duke ds project list', function(hooks) {
  ...
  test('it renders a list of projects', async function(assert) {
    ...
    assert.equal($('.duke-ds-project-name').eq(0).text().trim(), 'Project Name'); //header
    // index 1 is the search box
    assert.equal($('.duke-ds-project-name').eq(2).text().trim(), 'Project1');
    assert.equal($('.duke-ds-project-name').eq(3).text().trim(), 'Project2');
  });
  ...
  test('it sends selected project to selectionChanged action', async function(assert) {
    ...
    $('.duke-ds-project-name').eq(3).click(); //click Project 2 row
  });
});
